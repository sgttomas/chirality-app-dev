...
- Remaining naming/convention normalization items are governance/design follow-up and non-blocking for issued status.
...
|--------|--------|--------|
| **Publication Flow** | Constraints-based publication path is codified in `docs/CHANGE_PUBLICATION_GUIDANCE_CONSTRAINTS.md`. It requires: (a) approval-SHA binding per K-AUTH-2, (b) merge precondition per K-MERGE-1, and (c) flat hierarchy compliance per K-HIER-1. | `docs/CHANGE_PUBLICATION_GUIDANCE_CONSTRAINTS.md`; `docs/CONTRACT.md` K-AUTH-2, K-MERGE-1, K-HIER-1 |
...
| **Change Record Format** | CHANGE session logs are markdown records under `{EXECUTION_ROOT}/_Change/` snapshots and include: session identity, assumptions, state report, approval-SHA binding, approved actions, resulting state, and staleness triage advisories. Snapshot folders are immutable; `_LATEST.md` may be updated. | `agents/AGENT_CHANGE.md` Step 5; `docs/CONTRACT.md` K-SNAP-1 |
