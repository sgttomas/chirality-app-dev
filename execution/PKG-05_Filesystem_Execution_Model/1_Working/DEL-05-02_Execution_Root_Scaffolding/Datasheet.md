...
| **Package Folder Validity** | Named `{PKG-ID}_{PkgLabel}/`; contains `1_Working/`; SHOULD contain `0_References/`, `2_Checking/`, `3_Issued/` -- **[see CON-04 in Guidance Conflict Table for SHOULD vs MUST ruling]** | SPEC Section 12.2 |
...
| **Tool Root Sub-structure** | `_Aggregation/_Archive/`, `_Aggregation/_Templates/`, `_Decomposition/_Archive/` are required sub-directories; `_Sources/` sub-structure is TBD -- **[TBD_QUESTION (F-002):]** Check SPEC Section 1.2 for whether `_Sources/` requires internal sub-structure (e.g., `_Archive/`). | SPEC Section 1; **location TBD** for `_Sources/` detail |
...
|---------|-------------|--------|
| **Scaffolding Logic** | Code that creates the required execution-root directory tree (packages, tool roots, deliverable folders) when a new execution instance is initialized | **ASSUMPTION:** inferred from SOW-014 + SOW-015; implementation approach TBD |
...
| **Error Handling** | TBD -- **[TBD_QUESTION (B-002):]** Define error handling behavior when scaffolding encounters filesystem errors mid-operation (permissions, disk full, partial creation). See Specification REQ-12. | **ASSUMPTION:** Error handling behavior needs to be specified |
