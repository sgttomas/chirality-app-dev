# Tier 5 Control Loop Report — 2026-02-23 (Pass 12 DEL-03-05 Double-Encoded Redaction Hardening)
...
- Session objective: continue DEL-03-05 follow-through by closing REQ-09 leakage risk for double URL-encoded API-key variants in surfaced provider errors
...
| Control-loop intent | Extend PASS11 encoded-key redaction hardening to double URL-encoded leakage patterns while preserving existing multimodal boundaries |
...
- Redaction candidates now include:
  ...
  - single URL-encoded variants (upper/lowercase percent escapes + query-style `+` space form)
  - double URL-encoded variants (upper/lowercase percent escapes + query-style `+` space form)
- Expanded provider regression coverage in:
  - `frontend/src/__tests__/lib/harness-anthropic-agent-sdk-manager.test.ts`
- New coverage in this pass:
  - double URL-encoded key redaction in SDK status-error messages
  - double URL-encoded key redaction in stream `error` event messages
  - double URL-encoded key redaction in network error detail payloads
...
|---|---|---|---|
| R-T5-03 | Advanced multimodal coverage still depends on DEL-04-01 attachment-resolver maturity for resolver-integrated behavior beyond fixture-level contract tests | DEL-03-05, DEL-04-01 | MITIGATED (fixture-level provider boundary coverage expanded through PASS12) |
| R-T5-10 | Double URL-encoded API-key material can surface in upstream payloads and bypass single-encoding redaction sets | DEL-03-05 | CLOSED (double-encoded candidate redaction + SDK/stream/network regression coverage landed in PASS12) |
