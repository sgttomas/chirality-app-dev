...
| ItemID | LensKey | Type | AppliesToDoc | SuggestedEditDoc | CandidateInfo | WhyWarranted | SourcePath | SectionRef | Contenders | ProposedAuthority (PROPOSAL) | HumanRuling |
|---|---|---|---|---|---|---|---|---|---|---|---|
| D-001 | D:normative:applying | WeakStatement | Specification | Specification | Specify the expected value semantics of `HARNESS_PREMERGE_TEST_COUNT` (the current Guidance example shows `8` but the Specification does not mandate a fixed count); clarify whether this field is normative or informational | The enforced practice standard for machine-readable output (REQ-08) requires `HARNESS_PREMERGE_TEST_COUNT=<n>` but does not specify a fixed value; the current Guidance example shows `8` (6 section8 checks + 2 regression) but this is intentionally informational and can evolve | Specification.md, Guidance.md | Specification REQ-08, Guidance Example: Local Validation Run Sequence | -- | Specification.md | TBD |
